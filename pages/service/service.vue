  ...
  <view class="page" :style="pageStyle">
    ...
    <view class="hero-card" :style="cardStyle">
      ...
      <text class="hero-desc">
        {{ isCampusMode ? `当前学校：${currentCampus.name}` : '当前是普通模式' }}
      </text>
      <text class="hero-tip">
        {{ isCampusMode ? '会根据你当前选择的学校，自动匹配对应的校园生活服务。' : '切换到校园模式后，这里才会显示学校专属服务内容。' }}
      </text>
    ...
    <view v-else class="empty-card" :style="cardStyle">
      <text class="empty-title">{{ isCampusMode ? '当前学校暂未配置校园服务' : '校园服务仅在校园模式开放' }}</text>
      <text class="empty-desc">
        {{ isCampusMode ? '后面接入后台后，这里会按学校动态返回服务列表。' : '你可以先去校园切换页开启校园版，再回来查看校园专属服务。' }}
      </text>
    </view>
  </view>
</template>
